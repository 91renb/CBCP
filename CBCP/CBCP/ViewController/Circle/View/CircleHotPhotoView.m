...
@implementation CircleHotPhotoView
...
- (void)setPhotoArray:(NSArray *)photoArray
{
    ...
    if (photoArray.count == 1)
    {
        if (itemWidth == 100) {
            ...
            CircleHotImageModel *model = [photoArray firstObject];
    ...
    for (int i = 0; i < (photoArray.count >= 9 ? 9 : photoArray.count); i++) {
        CircleHotImageModel *model = photoArray[i];
...
- (CGFloat)itemWidthForPhotoArray:(NSArray *)phtotArray
{
    ...
    {
        CircleHotImageModel *model = [phtotArray firstObject];
...
{
    CircleHotImageModel *model = self.photoArray[index];
